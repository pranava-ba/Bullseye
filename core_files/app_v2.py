...
Fixes applied:
- All date inputs use format="DD/MM/YYYY" for universal display
- Date Range download caps to-date at today and warns about current-year downloads
- Download button logic skips future years/dates to prevent 404 loops
"""
import os
import time
...
_METHOD_LEGEND = dict(bgcolor="#161b22", bordercolor="#30363d", borderwidth=1)
...
def load_data(path: str) -> pd.DataFrame:
    # NSE CSV files are sometimes encoded in latin-1/cp1252 (not UTF-8).
    # Try UTF-8 first; fall back to latin-1 silently if it fails.
    try:
...
with st.sidebar:
    ...
    if download_mode == "Single Year":
        ...
        if dl_date_from > dl_date_to:
            ...
            # Cap to-date to today to prevent future-date download attempts
            ...
            # Warn if current year is in range
    ...
    if st.button("🚀 Start Download", type="primary", use_container_width=True):
        if not selected_years:
            ...
            # Filter out years that are entirely in the future
            valid_years = []
                ...
                for yr in valid_years:
                    with st.status(f"Downloading {yr}…", expanded=True) as _s:
                        ...
                        if ok:
                            ...
                            st.error(msg)
                        time.sleep(0.3)
                # Clear cache so re-downloaded data is loaded fresh
                st.cache_data.clear()
                st.session_state.auto_update_done = True
                st.rerun()
...
# Hard floor so the calendar widget can navigate to any historical year
# (e.g. 2018) even when a different year is currently loaded.
...
with tab2:
    ...
    with st.expander("✏️ Edit Pivot Values (override defaults for chart)", expanded=False):
        ...
        for sym in comp_symbols:
            ...
            st.markdown(f'<div class="section-title" style="margin-top:10px">{sym}</div>', unsafe_allow_html=True)
            for method in pivot_methods:
                ...
                st.markdown(f'<span class="method-badge {mc_badge}">{method}</span>', unsafe_allow_html=True)
    ...
    #"""Helper to format a table cell value."""
        if np.isnan(v):
    ...
    st.markdown(f'<div class="pivot-matrix-wrap"><table class="pivot-matrix">{thead}{tbody}</table></div>', unsafe_allow_html=True)
    ...
    if show_piv_chart:
        ...
        fig2.update_layout(
            **PLOTLY_LAYOUT,
            title=dict(text=f"{syms_lbl} — {meth_lbl} | {sel_piv_date}", font=dict(size=13, color="#e6edf3")),
            height=580,
            legend=dict(
                **_METHOD_LEGEND,
                orientation="h",
