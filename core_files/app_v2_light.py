"""
app_v2_light.py — NSE EOD Dashboard · Bullseye Fintech © 2024–2026
...
.stApp { background-color: #f4f5f9 !important; }
...
/* Popover overlay (the floating dropdown panel) */
...
[data-testid="stNumberInput"] input {
    ...
    font-weight: 600 !important;
    padding: 6px 48px 6px 8px !important; /* right padding to not overlap +/- btns */
    min-height: 38px !important;
}
/* The stepper +/- button wrapper */
...
/* ── Pivot edit expander — give each number input more breathing room ─────── */
...
/* Pivot edit text inputs — monospace numeric style */
...
/* Resistance / PP / Support colour-coded left border on text inputs */
.pivot-edit-res [data-testid="stTextInput"] input { border-left: 3px solid #e53935 !important; }
.pivot-edit-pp  [data-testid="stTextInput"] input { border-left: 3px solid #e65100 !important; background: #fffde7 !important; }
.pivot-edit-sup [data-testid="stTextInput"] input { border-left: 3px solid #43a047 !important; }
/* Legacy number input colours kept for any remaining uses */
.pivot-edit-res [data-testid="stNumberInput"] input { border-left: 3px solid #e53935 !important; }
.pivot-edit-pp  [data-testid="stNumberInput"] input { border-left: 3px solid #e65100 !important; background: #fffde7 !important; }
.pivot-edit-sup [data-testid="stNumberInput"] input { border-left: 3px solid #43a047 !important; }
...
.kpi-value {
    font-family: 'JetBrains Mono', monospace; font-size: 1.30rem;
    font-weight: 600; color: #1e2035 !important; line-height: 1.1;
}
.kpi-delta-pos { font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:#2e7d32 !important; margin-top:3px; }
.kpi-delta-neg { font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:#c62828 !important; margin-top:3px; }
.kpi-delta-neu { font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:#5e6180 !important; margin-top:3px; }
...
.wk52-title { font-family:'Sora',sans-serif; font-size:0.65rem; color:#5e6180 !important; letter-spacing:0.10em; text-transform:uppercase; margin-bottom:10px; }
.wk52-bar-outer { background:#e8eaf6; border-radius:6px; height:9px; position:relative; margin:8px 0 4px 0; }
.wk52-bar-fill { background:linear-gradient(90deg,#ef9a9a,#ffe082,#a5d6a7); border-radius:6px; height:9px; position:absolute; left:0; top:0; }
.wk52-marker { position:absolute; top:-4px; width:17px; height:17px; background:#fff; border-radius:50%; transform:translateX(-50%); border:2.5px solid #3949ab; box-shadow:0 0 6px rgba(57,73,171,0.28); }
.wk52-labels { display:flex; justify-content:space-between; font-family:'JetBrains Mono',monospace; font-size:0.72rem; color:#5e6180 !important; margin-top:6px; }
.wk52-cur { font-family:'JetBrains Mono',monospace; font-size:0.82rem; color:#3949ab !important; font-weight:600; }

/* ═══════════════════════════════════════════════════════════════════════════
   SECTION TITLE / INFO BOX / BADGES
   ═══════════════════════════════════════════════════════════════════════════ */
.section-title {
    font-family:'Sora',sans-serif; font-size:0.67rem; color:#5e6180 !important;
...
.info-box {
    ...
    font-size:0.82rem; color:#3949ab !important; margin-bottom:12px;
}
.info-box * { color:#3949ab !important; }
.method-badge { display:inline-block; border-radius:5px; padding:2px 9px; margin:1px 3px; font-family:'Sora',sans-serif; font-size:0.68rem; font-weight:600; letter-spacing:0.04em; }
...
/* All cells: visible dark border on every side */
.pivot-matrix th,
.pivot-matrix td {
    padding: 9px 14px; text-align: right;
    border: 1px solid #b0b7d4 !important;   /* ← dark grid lines everywhere */
    white-space: nowrap;
}
.pivot-matrix th:first-child, .pivot-matrix td:first-child { text-align: left; min-width: 60px; }
.pivot-matrix td:first-child { color: #3d4060 !important; font-size: 0.76rem; font-weight: 600; letter-spacing: 0.06em; }

/* Method header row */
.pivot-matrix thead tr.method-row th {
    font-family:'Sora',sans-serif; font-size:0.66rem;
...
.stTabs [data-baseweb="tab"] {
    font-family:'Sora',sans-serif; font-size:0.80rem; font-weight:500;
    letter-spacing:0.04em; color:#5e6180 !important; padding:10px 26px;
...
.stTabs [data-baseweb="tab-panel"] { background:transparent !important; padding-top:8px; }
...
.stDataFrame { font-family:'JetBrains Mono',monospace !important; font-size:0.80rem; }
...
[data-testid="stExpander"] summary p { color:#3949ab !important; font-family:'Sora',sans-serif !important; font-weight:600 !important; }

.stCaption, [data-testid="stCaptionContainer"] { color:#5e6180 !important; font-size:0.78rem !important; }
hr { border-color:#c5cae9 !important; }

/* ── Spinner ─────────────────────────────────────────────────────────────── */
[data-testid="stSpinner"] { color:#3949ab !important; }

/* ── Hide number-input stepper aria-label text (made visible by global p rule) */
...
/* ── Main header ─────────────────────────────────────────────────────────── */
.app-header { font-family:'Sora',sans-serif; font-weight:700; font-size:1.5rem; color:#1e2035 !important; letter-spacing:-0.01em; }
.app-header span { color:#3949ab !important; }

/* ═══════════════════════════════════════════════════════════════════════════
   DOWNLOAD LOG CARD  — explicit light-theme classes (immune to dark overrides)
   ═══════════════════════════════════════════════════════════════════════════ */
.dl-log-card {
    margin: 8px 0 4px;
    padding: 12px 14px;
...
.dl-log-meta {
    font-size: 0.74rem; color: #5e6180 !important;
    margin-bottom: 6px; font-weight: 500;
}
.dl-log-entry {
    font-size: 0.73rem; color: #3d4060 !important;
...
/* ═══════════════════════════════════════════════════════════════════════════
   PIVOT EXPANDER — method block header (prevents badge/input overlap)
   ═══════════════════════════════════════════════════════════════════════════ */
.pivot-method-header {
...
/* ═══════════════════════════════════════════════════════════════════════════
   GLOBAL CATCH-ALL — ensure no stray dark backgrounds leak through
   ═══════════════════════════════════════════════════════════════════════════ */
/* Any element with a near-black background colour injected by BaseWeb */
...
## KEYBOARD FIX — prevent arrow keys from navigating selectboxes/expanders unexpectedly.
# Streamlit's BaseWeb components intercept arrow keys globally for dropdown navigation.
# We block Left/Right arrow keys on text inputs (so typing doesn't close the pivot section),
# and block Up/Down + wheel on number inputs so they don't rerun the app.
st.markdown("""
<script>
(function() {
    ...
    document.addEventListener('keydown', function(e) {
        ...
        // On any text/number input: block all arrow keys from bubbling up to BaseWeb
        if (tag === 'INPUT' && (type === 'text' || type === 'number' || type === 'search' || type === '')) {
            if (_ALL_ARROWS.has(e.key)) {
                e.stopImmediatePropagation();
                // Also prevent Up/Down from stepping number inputs
                if (type === 'number' && _UPDOWN.has(e.key)) {
                    e.preventDefault();
                }
            }
        }
    }, true);   // capture=true → fires before any Streamlit/BaseWeb listener

    // Block scroll wheel on number inputs (prevents accidental value changes)
...
PLOTLY_LAYOUT = dict(
    ...
    plot_bgcolor="#fafbff",
    font=dict(family="Sora, sans-serif", color="#3d4060", size=12),
    xaxis=dict(gridcolor="#c5cae9", zeroline=False, color="#3d4060",
               tickfont=dict(color="#3d4060", size=11),
               title_font=dict(color="#1e2035", size=12)),
...
with st.sidebar:
    ...
    st.markdown(
        ...
        '<span style="font-size:0.70rem;font-weight:700;letter-spacing:0.10em;'
    ...
    if st.button("🚀 Start Download", type="primary", use_container_width=True):
        if not selected_years:
            ...
            if not valid_years:
                ...
                # Estimate trading dates (~252/year, prorated for current year)
                ...
                def _draw_prog_pct(pct_float, yr_label, ok, fail, est_d, finished=False):
                    ...
                    prog_ph.markdown(
                        ...
                        f'<span style="font-size:0.74rem;color:#5e6180;">{status_line}</span>'
                        f'</div>'
                        f'<div style="background:#dde3f0;border-radius:99px;height:12px;overflow:hidden;">'
                ...
                # Thread-safe result container and callback counter
                _cb_state = {"count": 0, "lock": threading.Lock()}
                _result_box: list = [None]  # [None] → (ok, msg) when done

                _draw_prog_pct(0, valid_years[0], 0, 0, est_dates)

                for idx, yr in enumerate(valid_years):
                    # Expected trading dates for this year (used for callback-based sub-progress)
                    yr_est = (252 if yr < current_year else
                              max(1, int((_date.today() - _date(yr, 1, 1)).days * 252 / 365)))
                    # Approximate seconds per year for time-based smooth animation
                    yr_est_secs = max(4.0, yr_est * 0.05)  # ~0.05s per trading date
                    ...
                    # Build per-year log lines in a local list; status widget writes them
                    with st.status(f"Downloading {yr}…", expanded=True) as _s:
                        _status_write = _s.write if hasattr(_s, "write") else st.write

                        def _make_cb(yr_label, yr_idx, yr_est_inner, writer):
                            """Callback fired by download_year_data once per trading date."""
                            def _cb(msg):
                                writer(msg)
                                with _cb_state["lock"]:
                                    _cb_state["count"] += 1
                                    cnt = _cb_state["count"]
                                # Use callback count for sub-progress; clamp at 98% leaving room for pivot step
                                sub_frac = min(cnt / max(yr_est_inner, 1), 0.98)
                                overall = (yr_idx + sub_frac) / total * 100
                                _draw_prog_pct(
                                    overall, yr_label,
                        ...
                        cb = _make_cb(yr, idx, yr_est, _status_write)

                        # Run download in background thread so we can animate the progress bar
                        ...
                        # Animate progress while thread is running
                        t_start = time.monotonic()
                        last_pct = idx / total * 100
                        while dl_thread.is_alive():
                            elapsed = time.monotonic() - t_start
                            # Time-based interpolation: smoothly approach (idx+0.95)/total
                            # Uses exponential ease-out so it slows as it approaches the cap
                            time_frac = 1.0 - 2.0 ** (-elapsed / max(yr_est_secs / 3, 1.0))
                            time_based_pct = (idx + time_frac * 0.95) / total * 100
                            # Use whichever is higher: callback-based or time-based
                            with _cb_state["lock"]:
                                cb_cnt = _cb_state["count"]
                            cb_frac = min(cb_cnt / max(yr_est, 1), 0.98)
                            cb_pct = (idx + cb_frac) / total * 100
                            display_pct = max(last_pct, min(time_based_pct, cb_pct if cb_cnt > 0 else time_based_pct))
                            ...
                            time.sleep(0.25)  # refresh ~4 fps — smooth without hammering Streamlit
                ...
                # Store final pct for persistent display
    ...
    # ── Persistent download summary — survives rerun ────────────────────────────
    if st.session_state.get("dl_log"):
        ...
        summ   = (f"✅ {ok_c}/{tot_c} years · ~{est_d:,} trading dates"
                  if fail_c == 0
        ...
        st.markdown(
            ...
            f'<span style="font-size:0.74rem;color:#5e6180;">{summ}</span>'
            f'</div>'

            f'<div style="background:#dde3f0;border-radius:99px;height:12px;overflow:hidden;margin-bottom:10px;">'
...
if df_raw is None:
    st.markdown(
        '<div class="app-header">📈 <span>Bullseye Fintech</span> — NSE EOD Dashboard</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<div class="info-box" style="margin-top:18px;">Open the sidebar (≡ top-left) '
        'and use <strong>Download from NSE</strong> to get started.</div>',
        unsafe_allow_html=True,
    )
    st.stop()
...
with tab1:
    ...
    fig.update_layout(
        **PLOTLY_LAYOUT,
        title=dict(text=f"{detail_sym} — {chart_type}", font=dict(size=13, color="#1e2035")),
        ...
        legend=dict(
            **_METHOD_LEGEND,
            ...
            x=0, xanchor="left",
            y=-0.12, yanchor="top",
            font=dict(size=10, color="#5e6180"),
            tracegroupgap=6,
            itemwidth=40,
        ),
    )
    fig.update_xaxes(showgrid=True, gridcolor="#c5cae9", color="#3d4060", tickfont=dict(color="#3d4060", size=11))
    fig.update_yaxes(showgrid=True, gridcolor="#c5cae9", color="#3d4060", tickfont=dict(color="#3d4060", size=11))
    ...
    if warn_parts:
        st.markdown(
            ...
            f'<span style="color:#8b8fa8;font-size:0.78rem;">Widen the date range to see them.</span></div>',
            unsafe_allow_html=True,
        )
...
with tab2:
    ...
    # Uses a button-toggle instead of st.expander to avoid BaseWeb's global
    # arrow-key listener that fires when the expander is open.
    _exp_key = "pivot_edit_open"
    if _exp_key not in st.session_state:
        st.session_state[_exp_key] = False

    _btn_label = "🔼 Close Edit Pivot Values" if st.session_state[_exp_key] else "✏️ Edit Pivot Values (override defaults for chart)"
    if st.button(_btn_label, key="pivot_edit_toggle", use_container_width=False):
    ...
    if st.session_state[_exp_key]:
        st.markdown(
            '<div class="info-box">📌 Pre-filled with computed defaults. '
            'Edit any value to override it on the chart. '
        ...
        for sym in comp_symbols:
            ...
            st.markdown(
                f'<div class="section-title" style="margin-top:14px;margin-bottom:8px;">{sym}</div>',
                unsafe_allow_html=True,
            )
            for method in pivot_methods:
                ...
                # Method header
                st.markdown(
                    f'<div style="margin:12px 0 10px 0;padding:6px 12px;'
                    f'border-radius:6px;border-left:4px solid '
                    f'{"#3949ab" if method=="Classic" else "#bf360c" if method=="Fibonacci" else "#1b5e20" if method=="Woodie" else "#4a148c"}'
                    f';background:{"#eef0fb" if method=="Classic" else "#fff8e1" if method=="Fibonacci" else "#e8f5e9" if method=="Woodie" else "#f3e5f5"}'
                    f'"><strong style="font-size:0.80rem;">{method}</strong>'
                    f'<span style="font-size:0.72rem;margin-left:8px;opacity:0.7;">— edit levels below</span></div>',
                    unsafe_allow_html=True,
                )

                # Build level list: R levels highest first, then PP, then S levels
                r_levels = [(f"R{j+1}", rc, "res") for j, rc in enumerate(r_cols[:n_levels])]
                pp_level = [("PP", pp_col, "pp")]
                s_levels = [(f"S{j+1}", sc, "sup") for j, sc in enumerate(s_cols[:n_levels])]
                levels = list(reversed(r_levels)) + pp_level + s_levels

                # Render in rows of up to 4 columns using text_input (no arrow-key capture)
                MAX_COLS = 4
                for chunk_start in range(0, len(levels), MAX_COLS):
                    chunk = levels[chunk_start:chunk_start + MAX_COLS]
                    edit_cols = st.columns(len(chunk))
                    for ci, (lbl, col_key, kind) in enumerate(chunk):
                        default_v = (
                            float(row.get(col_key, 0.0))
                            if (row is not None and pd.notna(row.get(col_key, np.nan)))
                            else 0.0
                        )
                        ov_key = f"ov_{sym}_{method}_{col_key}"
                        prefix = {"res": "🔴", "pp": "🟠", "sup": "🟢"}[kind]
                        with edit_cols[ci]:
                            raw = st.text_input(
                                f"{prefix} {lbl}",
                                value=f"{default_v:.2f}",
                                key=ov_key,
                                help=f"Default: {default_v:,.2f}",
                            )
                        try:
                            new_val = float(raw.replace(",", "").strip())
    ...
    if show_piv_chart:
        ...
        fig2.update_layout(
            **PLOTLY_LAYOUT,
            title=dict(text=f"{syms_lbl} — {meth_lbl} | {sel_piv_date}", font=dict(size=13, color="#1e2035")),
            height=580,
            legend=dict(
                **_METHOD_LEGEND,
                orientation="h",
        ...
        fig2.update_xaxes(showgrid=True, gridcolor="#c5cae9", color="#3d4060", tickfont=dict(color="#3d4060", size=11))
        fig2.update_yaxes(showgrid=True, gridcolor="#c5cae9", color="#3d4060", tickfont=dict(color="#3d4060", size=11))
